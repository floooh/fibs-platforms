    ...
    configs: [
        {
            ...
            validate: (project: fibs.Project) => {
                if (!fibs.util.dirExists(dir(project))) {
                    return {
                        valid: false,
                        hints: [ "WASI SDK not installed (install with 'fibs wasisdk install')" ],
                    };
                } else {
...
async function runnerRun(project: fibs.Project, config: fibs.Config, target: fibs.Target, options: fibs.RunOptions) {
...
function parseArgs(): Args {
    ...
    switch (Deno.args[1]) {
        ...
        case 'uninstall':
            ...
            break;
        default:
            fibs.log.error(`unknown subcommand '${Deno.args[1]} (run 'fibs help wasisdk')`);
    }
    return args;
}
...
async function download(project: fibs.Project) {
    if (fibs.util.dirExists(dir(project))) {
        fibs.log.error(`WASI SDK already installed, run 'fibs wasisdk uninstall' first`);
    }
