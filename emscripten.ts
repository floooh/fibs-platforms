    ...
    configs: [
        {
            ...
            validate: (project: fibs.Project) => {
                if (!fibs.util.dirExists(dir(project))) {
                    return {
                        valid: false,
                        hints: [ "Emscripten SDK not installed (install with 'fibs emsdk install')" ],
                    };
                } else {
...
async function runnerRun(project: fibs.Project, config: fibs.Config, target: fibs.Target, _options: fibs.RunOptions) {
...
function parseArgs(): { install?: boolean, list?: boolean, uninstall?: boolean, version?: string } {
    ...
    switch (Deno.args[1]) {
        ...
        case 'uninstall':
            ...
            break;
        default:
            fibs.log.error(`unknown subcommand '${Deno.args[1]} (run 'fibs help emsdk')`);
    }
    return args;
}
...
async function emsdk(project: fibs.Project, args: string[]): Promise<number> {
    ...
    if (!fibs.util.fileExists(cmd)) {
        fibs.log.error(`emsdk tool not found at ${toolPath}, run 'fibs emsdk install`);
    }
    const res = await fibs.util.runCmd(cmd, { args, cwd: dir(project), winUseCmd: true });
    return res.exitCode;
}

async function install(project: fibs.Project, version: string) {
    await cloneOrUpdateEmsdk(project);
    await emsdk(project, ['install', '--shallow', '--disable-assertions', version]);
